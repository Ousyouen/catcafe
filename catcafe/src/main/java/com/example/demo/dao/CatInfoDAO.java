...
public interface CatInfoDAO {
    ...
    // 名前と年齢で検索（模糊查询）
    @Select("SELECT * FROM cat_info WHERE cat_name LIKE CONCAT('%', #{name}, '%') "
            + "AND (cat_age = #{age} OR #{age} IS NULL) AND delete_flag = 0")
    List<CatInfoMst> searchCatsByNameAndAge(@Param("name") String name, @Param("age") Integer age);

    // 名前で検索（模糊查询）
    @Select("SELECT * FROM cat_info WHERE cat_name LIKE CONCAT('%', #{name}, '%') AND delete_flag = 0")
    List<CatInfoMst> searchCatsByName(@Param("name") String name);

    // 年齢で検索
    @Select("SELECT * FROM cat_info WHERE cat_age = #{age} AND delete_flag = 0")
